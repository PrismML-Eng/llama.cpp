...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_load_tiles_pq1_0(
        const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int warp_size   = ggml_cuda_get_physical_warp_size();
    constexpr int nwarps      = ggml_cuda_mmq_get_nthreads(type, J, fallback) / warp_size;
    constexpr int I           = ggml_cuda_mmq_get_I(type, J, fallback);
    ...
    constexpr int blocks_per_iter = MMQ_ITER_K / QK_PQ1_0;
    constexpr int threads_per_row = blocks_per_iter * QI_PQ1_0;
    constexpr int nrows = warp_size / threads_per_row;
    constexpr int scale_entries_per_block = QK_PQ1_0 / QK8_1;
    constexpr int scale_entries_per_row = blocks_per_iter * scale_entries_per_block;

    const int txi  = threadIdx.x % threads_per_row;
    const int kbx  = txi / QI_PQ1_0;
    const int kqsx = txi % QI_PQ1_0;
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nrows*nwarps) {
        ...
        const block_pq1_0 * bxi = (const block_pq1_0 *) x + kbx0 + i*stride + kbx;
        const int16_t    * qxi = (const int16_t *) bxi->qs + kqsx * 2;
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps) {
        ...
        const block_pq1_0 * bxi = (const block_pq1_0 *) x + kbx0 + i*stride + scale_block;
