...
// Q1_0 multi-column mat-vec, half2 form. Two src1 columns share one half2 lane, so
// a single select+add covers both and the ALU slots per weight per column halve --
// the multi-column kernel above is ALU-bound (2 fp32 lane-ops per weight per column),
// not weight-read-bound, at the 2..8 rows a speculative verify batch uses.
// Only the per-row accumulator is half: the block sum of y stays fp32 (it is computed
// once per block for all nr0 rows, so its cost is amortized) and the accumulator is
// promoted to fp32 at every q1_0 block, bounding the rounding to at most SW halves.
// Opt-in via GGML_METAL_Q1_0_H2=1; gate on measured logit-KLD before enabling by default.
template<int nr0, int npair, int tpb, typename args_t>
void kernel_mul_mv_q1_0_f32_h2_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    half2 yh[npair][SW];
    float sumy[2*npair];
    float sumf[nr0][2*npair];
    FOR_UNROLL (short row = 0; row < nr0; row++) {
        FOR_UNROLL (short c = 0; c < 2*npair; c++) {
            sumf[row][c] = 0.f;
        }
    }
    ...
    for (int ib = ix; ib < nb; ib += N_SIMDWIDTH/tpb) {
        FOR_UNROLL (short p = 0; p < npair; p++) {
            float s0 = 0.f;
            float s1 = 0.f;
            FOR_UNROLL (short i = 0; i < SW; i++) {
                const float y0 = yb[2*p    ][i];
                const float y1 = yb[2*p + 1][i];
                yh[p][i] = half2((half) y0, (half) y1);
                s0 += y0;
                s1 += y1;
            }
            sumy[2*p    ] = s0;
        ...
        FOR_UNROLL (short row = 0; row < nr0; row++) {
            ...
            FOR_UNROLL (short p = 0; p < npair; p++) {
                half2 acc = half2(0.0h);

                FOR_UNROLL (short i = 0; i < SW; i++) {
                    acc += bool(b[i/8] & (1u << (i%8))) ? yh[p][i] : half2(0.0h);
                }

                sumf[row][2*p    ] += d*(2.0f*(float) acc.x - sumy[2*p    ]);
                sumf[row][2*p + 1] += d*(2.0f*(float) acc.y - sumy[2*p + 1]);
            }
        }
...
[[host_name("kernel_mul_mv_q1_0_f32_h2_nr1_2")]]
kernel void kernel_mul_mv_q1_0_f32_h2_nr1_2(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mv_q1_0_f32_h2_impl<N_R0_Q1_0, 1, 8, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, nullptr, tgpig, tiisg, sgitg);
}

[[host_name("kernel_mul_mv_q1_0_f32_h2_nr1_4")]]
kernel void kernel_mul_mv_q1_0_f32_h2_nr1_4(
        constant ggml_metal_kargs_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mv_q1_0_f32_h2_impl<N_R0_Q1_0, 2, 16, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, nullptr, tgpig, tiisg, sgitg);
}
