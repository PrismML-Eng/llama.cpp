...
inline float block_q_n_dot_y(device const block_pq1_0 * qb_curr, float sumy, thread float * yl, int il) {
    device const uint8_t * qs = qb_curr->qs + il / 8;
    const uint8_t b0 = qs[0];
    const uint8_t b1 = qs[1];
...
template<int nr0, typename args_t>
void kernel_mul_mv_pq1_0_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    const int nb = args.ne00/QK_PQ1_0;
    ...
    device const block_pq1_0 * ax[nr0];
    for (int row = 0; row < nr0; ++row) {
        const uint64_t offset0 = (first_row + row)*args.nb01 + (i12/FC_mul_mv_r2)*args.nb02 + (i13/FC_mul_mv_r3)*args.nb03;
        ax[row] = (device const block_pq1_0 *) ((device char *) src0 + offset0);
    }
    ...
    device const float * yb = y + ix*QK_PQ1_0 + il;
...
[[host_name("kernel_mul_mv_pq1_0_f32")]]
kernel void kernel_mul_mv_pq1_0_f32(
    ...
    kernel_mul_mv_pq1_0_f32_impl<N_R0_PQ1_0, constant ggml_metal_kargs_mul_mv &>(args, src0, src1, dst, nullptr, tgpig, tiisg, sgitg);
}
...
template [[host_name("kernel_mul_mv_id_pq1_0_f32")]]   kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_pq1_0_f32_impl<N_R0_PQ1_0>>>;
