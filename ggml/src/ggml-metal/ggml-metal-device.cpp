...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_gated_delta_net(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    // in-place state update requested by the graph builder (plain decode, K=1) -- the kernel
    // writes the final state back into the state input (recurrent-cache view) instead of the
    // dst tail. See llm_build_delta_net_base::build_recurrent_attn.
    const bool inplace = op->op_params[0] != 0;

    // f16 state: supported only together with the in-place path (the non-inplace path routes
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_rmsnorm_qmv(ggml_metal_library_t lib, enum ggml_type qtype) {
    GGML_ASSERT(qtype == GGML_TYPE_Q1_0 || qtype == GGML_TYPE_Q2_0);

    char base[256];

    snprintf(base, 256, "kernel_rmsnorm_mv_%s_f32", qtype == GGML_TYPE_Q1_0 ? "q1_0" : "q2_0");
