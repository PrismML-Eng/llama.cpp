...
void process_shaders() {
    ...
    for (const auto& tname : type_names) {
        ...
        if (tname != "f16" && tname != "bf16") {
            const std::string dequant_shader = tname == "q1_0_g128" ? "dequant_q1_0.comp" : "dequant_" + tname + ".comp";
            string_to_spv("dequant_" + tname, dequant_shader, merge_maps(base_dict, {{data_a_key, "1"}, {"D_TYPE", "float16_t"}}));
        }
    ...
    for (std::string t : {"q1_0", "q1_0_g128", "q2_0", "q4_0", "q4_1", "q5_0", "q5_1", "q8_0", "iq4_nl"}) {
        string_to_spv("cpy_f32_" + t, "copy_to_quant.comp", {{"DATA_A_" + to_uppercase(t), "1"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}});
        string_to_spv("cpy_" + t + "_f32", "copy_from_quant.comp", {{"DATA_A_" + to_uppercase(t), "1"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}});
    }

    for (std::string t : {"f32", "f16", "bf16", "q1_0", "q1_0_g128", "q2_0", "q4_0", "q4_1", "q5_0", "q5_1", "q8_0", "iq4_nl"}) {
