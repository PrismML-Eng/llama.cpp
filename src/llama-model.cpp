...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    if (!hadamard_weight_blocks.empty() || !hadamard_inverse_blocks.empty()) {
        ...
        // keep lookup-side transforms on the device of the forward rotations,
        // not on the host buffer of a CPU-mapped embedding table
        ggml_backend_buffer_type_t preferred_buft = nullptr;

        const std::unordered_map<std::string, uint32_t> * group_blocks[2] = { &hadamard_weight_blocks, &hadamard_inverse_blocks };
        llama_hadamard_rotations *                        group_target[2] = { &hadamard_rotations,     &hadamard_inverses      };

        for (int g = 0; g < 2; ++g) {
            llama_hadamard_rotations * target = group_target[g];
            for (const auto & entry : *group_blocks[g]) {
